...
const generateDataPageUrl = (value) => `https://www.gismeteo.ru${value}3-days/#2-4-days `;
...
function getForecastWeather(city) {
    let searchPageByCityUrl = encodeURI(SEARCH_PAGE_URL + city);
    let res = request('GET', searchPageByCityUrl, {});
    let htmlSearchPage = cheerio.load(res.body);
    let href = htmlSearchPage(HREF_SELECTOR).attr("href");
    let dataPageUrl = generateDataPageUrl(href);
    let rest = request('GET', dataPageUrl, {});
